...
- [x] 详情页：大图 + 文件名 / 拍摄时间 / 图片大小 / 视频大小 / 总计 + EXIF 信息（有理数格式化）
- [x] 长按预览：详情页大图上长按 → `video_player` 播放本地视频 → 松手停止回静态图；无控制栏、不跳播放器
- [ ] 删除流程：入口 → 系统确认（回收站）→ 成功提示 → 列表刷新；错误/取消状态处理（实现中）

> Phase 1/Phase 2 完成于 2026-08-05；真机验证：2166 张 Live 图片、21.97 GB。
> Phase 3 长按播放与详情页完成于 2026-08-06，真机验证通过（截图归档于 `build/`）。
> 备注：Kotlin 增量编译因 C:/D: 跨盘缓存损坏已关闭（`android/gradle.properties`）。

## 6. 已知问题（待解决）

以下问题已记录，暂不阻塞 MVP 主线，后续统一处理：

1. **缩略图生成慢（vivo）**：依赖系统 `loadThumbnail`，首屏缩略图逐格生成较慢，滚动时会出现灰色占位。
   候选方案：网格可见区域批量预生成、自研降采样解码（注意：vivo 上手动 `BitmapFactory` 二次解码曾全部失败，需先验证）。
2. **详情页首次打开等待**：原图需复制到缓存后才显示（已先用缩略图占位），首次打开个别照片仍有感知延迟；打开后缓存生效。
3. **vivo 重装权限确认弹窗**：`adb install -r` 更新时手机弹出“权限确认”，自动化安装会被拒绝，需要人工点允许。
4. **开发工具限制**：uiautomator 在滚动时会返回陈旧界面树（曾误导滚动验证）；沙箱内 git 因目录所有权需提权执行。
5. **删除功能未真机验证**：按用户要求，删除（含系统回收站弹窗）必须在用户明确确认后测试。
