...
LiveKit 是一个本地运行的 Android Live Photo 清理工具。

> 当前 v0.1.0 版本主要适合使用 vivo 传统双文件 Live Photo 的用户。也就是照片目录里能看到同名的 `.jpg/.jpeg` 和 `.mp4` 文件。
>
> vivo 最新机型如果已经适配单文件 Motion Photo 格式，例如 X300 系列及以上、S50 系列及以上，以及其他发布时间晚于 X300 系列的机型，v0.1.0 暂不支持清理这类 Live Photo。最新 vivo 机型和其他品牌的 Motion Photo 兼容正在适配中。

它的核心用途不是替代系统相册，而是帮你清理 Live Photo 里占空间的“动态视频部分”，同时尽量保留原来的静态照片。
...
很多人旅行、聚会、拍孩子、拍宠物时会打开 Live Photo，因为它能记录按下快门前后的一小段动态，偶尔非常有用。

但真实使用里也经常出现另一种情况：

- 只是忘了关 Live 模式
- 连续拍了很多普通照片，但每张都多出一段动态视频
- 这些动态片段大部分以后不会再看
- 真正需要保留的往往只是那张静态照片
- 时间久了，手机空间被这些隐藏的视频慢慢占满

系统相册通常不会提供一个足够直接的方式，让你批量检查并清理这些 Live Photo 的动态部分。第三方相册还可能把 JPG 和 MP4 拆开显示，管理起来更麻烦。
...
v0.1 主要支持 vivo 传统 Live Photo 双文件结构，不支持已经改用单文件 Motion Photo 的新机型：
...
也就是同一个目录下，静态 JPG/JPEG 和动态 MP4 文件名相同，只是扩展名不同。
...
LiveKit 会把它们识别成一张 Live Photo，而不是两个独立文件。
...
```text
清理 Live 动态部分 = 处理同名 MP4
静态 JPG 会保留
```

这样照片仍然存在，只是不再保留动态效果。适合清理那些“只是因为忘记关闭 Live 模式而产生”的动态视频。
...
- 经常拍 Live Photo，但很多动态效果其实不需要
- 手机空间越来越少，想清理 Live Photo 占用的视频部分
- 想保留照片本身，不想把整张照片删掉
- vivo 手机里有大量 `JPG + 同名 MP4` 的 Live Photo
- 希望先看清空间占用，再决定是否清理

如果你只是想找一个完整相册替代品，LiveKit 不是这个方向。它更像一个专门处理 Live Photo 动态部分的轻量工具。
